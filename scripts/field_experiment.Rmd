...
```{r}
table(filter(field, !duplicated(ID))$Name)
```
...
About the same. These can safely be lumped together:

```{r}
field <- mutate(field,
                Language = ifelse(Language %in% c('BE', 'US'),
                                  'EN', Language))
...
```{r}
str_c(round(mean(field$ACC), 2) * 100, '%')
```

In comparison, chance is:

```{r}
str_c(round(1/12, 2) * 100, '%')
```
...
These need to be corrected for performing 15 correlations:

```{r}
# Vector of adjusted p-values from upper triangle of the matrix:

adjusted_p <- p.adjust(p_vals[upper.tri(p_vals)], method = 'bonferroni', n = 15)

# Replace lower triangle with NAs to avoid confusion:

p_vals[lower.tri(p_vals)] <- NA

# Save adjusted p-values back into upper triangle:

p_vals[upper.tri(p_vals)] <- adjusted_p
...
The only ones that don't "significantly" correlate with each other after correction are SR ~ BR and SR ~ PA. All others are correlated, even if correcting for 15 tests.
